...
<script lang="ts">
  ...
  function toggleYear(yearId: string) {
    goalExpandedYears.update((state) => ({
      ...state,
      [yearId]: !state[yearId],
    }));
  }
    ...
    {#if $goalExpandedYears[year.id]}
      <div class="ml-12 mt-2 space-y-2">
        {#each year.months as month (month.id)}
          ...
          {@const monthBothCompleted =
            month.priGoalCompleted && month.proGoalCompleted}
          {@const monthBothRejected =
            month.priGoalRejected && month.proGoalRejected}
          {@const monthMixed =
            (month.priGoalCompleted || month.priGoalRejected) &&
            (month.proGoalCompleted || month.proGoalRejected) &&
            !monthBothCompleted &&
            !monthBothRejected}
          {@const monthBorderColor = monthBothCompleted
          ...
          {#if $goalExpandedMonths[monthKey]}
            <div class="ml-12 mt-2 space-y-2 bg-white/10">
              {#each month.weeks as week (week.id)}
                ...
                {@const weekBothCompleted =
                  week.priGoalCompleted && week.proGoalCompleted}
                {@const weekBothRejected =
                  week.priGoalRejected && week.proGoalRejected}
                {@const weekMixed =
                  (week.priGoalCompleted || week.priGoalRejected) &&
                  (week.proGoalCompleted || week.proGoalRejected) &&
                  !weekBothCompleted &&
                  !weekBothRejected}
                {@const weekBorderColor = weekBothCompleted
                  ? "border-2 border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.8)]"
                  : weekBothRejected
                    ? ""
                    : weekMixed
                  ...
                  {#if $goalExpandedWeeks[weekKey] && week.days}
                    <div class="ml-12 mt-3 space-y-3">
                      {#each week.days as day (day.id)}
                        {@const bothCompleted =
                          day.priGoalCompleted && day.proGoalCompleted}
                        {@const bothRejected =
                          day.priGoalRejected && day.proGoalRejected}
                        {@const mixed =
                          (day.priGoalCompleted || day.priGoalRejected) &&
                          (day.proGoalCompleted || day.proGoalRejected) &&
                          !bothCompleted &&
                          !bothRejected}
