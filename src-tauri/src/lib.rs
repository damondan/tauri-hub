    ...
    for alert in new_alerts {
        if let Some(level) = parse_alert_level(&alert) {
            let (title, body) = if level >= 12 {
                (
                    "🚨 Critical OSSEC Alert",
                    format!("High severity alert detected (Level {})!", level),
                )
            } else if level >= 8 {
                (
                    "⚠️ OSSEC Security Alert",
                    format!("Suspicious activity detected (Level {}).", level),
                )
            } else if level >= 5 {
                (
                    "📋 OSSEC Alert",
                    format!("Alert detected (Level {}).", level),
                )
            } else {
...
async fn toggle_lmstudio(start: bool) -> Result<(), String> {
    if start {
        ...
        if !std::path::Path::new(&appimage_path).exists() {
            return Err(format!(
                "LM Studio AppImage not found at: {}",
                appimage_path
            ));
        }
...
fn get_gpu_usage() -> Result<(f64, f64, f64), String> {
    ...
    if parts.len() >= 2 {
        let used_mb: f64 = parts[0]
            .parse()
            .map_err(|_| "Failed to parse used memory")?;
        let total_mb: f64 = parts[1]
            .parse()
...
async fn load_user_data(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    fs::read_to_string(&data_path).map_err(|e| format!("Failed to read user data: {}", e))
}
...
async fn save_user_data(data: String, app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    fs::write(&data_path, data).map_err(|e| format!("Failed to write user data: {}", e))
}
//check save
#[tauri::command]
async fn save_user_data_encryption(
    data: String,
    app_handle: tauri::AppHandle,
) -> Result<(), String> {
    let data_path = get_user_data_path_encryption(&app_handle)?;

    fs::write(&data_path, data).map_err(|e| format!("Failed to write user data encryption: {}", e))
}
...
async fn backup_user_data(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    fs::copy(&data_path, &backup_path).map_err(|e| format!("Failed to backup user data: {}", e))?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let registry_data = load_registry(&app.handle()).unwrap_or_else(|e| {
